...
function TournamentProfileInfo({type = "global", tournamentId = null}) {
        ...
        async function fetchData() {
            ...
            try {
                ...
                if (type === "tournament" && tId) {
                    ...
                    const totalGoals = matches.reduce((acc, m) => {
                        if (m.status === "FINISHED") {
                            return acc + (m.homeScore || m.teamHomeScore || 0) + (m.awayScore || m.teamAwayScore || 0);
                        }
                        return acc;
                    }, 0);

                    const finishedGames = matches.filter(m => m.status === "FINISHED").length;

                    setData({
                        title1: "Teams", val1: tour.teams?.length || 0,
                        title2: "Players", val2: tour.totalPlayers || 0,
                        title3: "Games", val3: `${finishedGames}/${matches.length}`,
                        title4: "Goals", val4: totalGoals || 0
                    });
                }
                // PRIORIDAD 2: ADMIN GLOBAL
                else if (type === "global" && user.isAdmin) {
                    const [resTournaments, resTeams, resUsers, resMatches] = await Promise.all([
                    ...
                    setData({
                        title1: "Tournaments", val1: resTournaments.data.length,
                        title2: "Teams", val2: resTeams.data.length,
                        title3: "Players", val3: resUsers.data.length,
                        title4: "Total Goals", val4: totalSystemGoals
                    });
                }
                // PRIORIDAD 3: USUARIO COMÚN
                    ...
                    const resTour = await axios.get(`${API}/tournaments/${searchTourId}`, config);
                    ...
                    const myRankIndex = allTeams.findIndex(t => String(t.id) === String(user.teamId));
                    ...
                    const positionDisplay = myRankIndex !== -1 ? `${myRankIndex + 1}º` : "-";

                    setData({
                        val1: myTeamData.imgProfile || null,   title1: "Team",

                        // AQUI ESTÁ LA MAGIA:
                        val2: positionDisplay,                 title2: "Position",

                        val3: myTeamData.goalsFor || 0,        title3: "Goals",
                        val4: myTeamData.matchesPlayed || 0,   title4: "Games"
                    });
                } else {
                    setData(null);
                }
    ...
    return (
        <div className="tournament-info">
            <div className={styles.info__tournament_section}>
                {/* CAJA 1: Team (Imagen) */}
                <article>
                    {/* Si el título es "Team", mostramos la imagen, si no, texto */}
                    {data.title1 === "Team" && data.val1 ? (
                        <div style={{width: '40px', height: '40px', borderRadius: '50%', overflow: 'hidden', margin: '0 auto'}}>
                            <img
                                src={data.val1}
                                alt="Team"
