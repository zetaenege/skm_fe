...
function NextMatchLive({ tournamentId, onMatchFinished }) {
  ...
  const handleStart = () => setGameStatus("LIVE");
  const handleStop = () => setGameStatus("EDIT");
  ...
  if (loading) return <p>Loading next match...</p>;
      ...
      <article className={style.next__match_header}>
        ...
        <div>
          <div className={style.match__state}>
            {gameStatus === "PRE" && (
              <img src={nextMatchIcon} alt="Matchday" className={style.icon__svg} />
            )}
            {gameStatus === "LIVE" && (
              <img src={matchPlayingIcon} alt="Live Match" className={style.icon__svg} />
            )}
            {gameStatus === "EDIT" && (
              <img src={matchFinishedIcon} alt="Finished Match" className={style.icon__svg} />
            )}
          </div>
        </div>
      </article>
              ...
              <div className={style.score__controls_panel}>
                <div className={style.control__group}>
                  <div className={style.buttons__row}>
                    ...
                    >
                      <img src={minusIcon} alt="Minus" className={style.icon__svg} />
                    </button>
                    <button
                      className={style.btn__score}
          ...
          <div className={style.team__img_name}>
            <div className={`animate__item delay_1 ${style.team__img}`}>
              <img
                ...
                alt=""
                className={
                  !match.awayTeam?.imgProfile ? style.icon__style : style.full__img
                }
              />
            </div>
            <p className="text_name_small animate__item delay_2">
